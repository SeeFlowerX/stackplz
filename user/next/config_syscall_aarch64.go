...
func init() {
	...
	var INT = GetArgType(TYPE_INT)
	// var UINT = GetArgType(TYPE_UINT)
	// var INT8 = GetArgType(TYPE_INT8)
	var INT16 = GetArgType(TYPE_INT16)
	// var INT32 = GetArgType(TYPE_INT32)
	// var INT64 = GetArgType(TYPE_INT64)
	// var UINT8 = GetArgType(TYPE_UINT8)
	// var UINT16 = GetArgType(TYPE_UINT16)
	// var UINT32 = GetArgType(TYPE_UINT32)
	// var UINT64 = GetArgType(TYPE_UINT64)
	// var POINTER = GetArgType(TYPE_POINTER)
	var STRING = GetArgType(TYPE_STRING)
	var BUFFER = GetArgType(TYPE_BUFFER)
	// var STRUCT = GetArgType(TYPE_STRUCT)
	var IOVEC = GetArgType(TYPE_IOVEC)
	var MSGHDR = GetArgType(TYPE_MSGHDR)
	var SOCKADDR = GetArgType(TYPE_SOCKADDR)
	// var TIMESPEC = GetArgType(TYPE_TIMESPEC)
	// // 对一些复杂结构体的读取配置进行补充
	...
	R(56, "openat", A("dirfd", INT), A("*pathname", STRING), A("flags", INT), A("mode", INT16))
	R(66, "writev", A("fd", INT), A("*iov", IOVEC_X2), A("iovcnt", INT))
	R(206, "sendto", A("sockfd", INT), A("*buf", BUFFER_X2), A("len", INT), A("flags", INT), A("addr", SOCKADDR), A("addrlen", INT))
