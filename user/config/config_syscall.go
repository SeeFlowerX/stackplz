...
func init() {
	...
	Register(&SArgs{270, PA("process_vm_readv", []PArg{A("pid", INT), B("local_iov", IOVEC), A("liovcnt", INT), B("remote_iov", IOVEC), A("riovcnt", INT), A("flags", INT)})})
	Register(&SArgs{271, PA("process_vm_writev", []PArg{A("pid", INT), A("local_iov", IOVEC), A("liovcnt", INT), A("remote_iov", IOVEC), A("riovcnt", INT), A("flags", INT)})})
